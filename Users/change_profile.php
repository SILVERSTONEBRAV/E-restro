...
if (isset($_POST['ChangeProfile'])) {
  ...
  if (empty($_POST["customer_phoneno"]) || empty($_POST["customer_name"]) || empty($_POST['customer_email'])) {
    ...
    $postQuery = "UPDATE m_customers SET customer_name =?, customer_phoneno =?, customer_email =?, customer_password =? WHERE  customer_id =?";
...
if (isset($_POST['changePassword'])) {
    ...
    if (!$error) {
        $customer_id = $_SESSION['customer_id'];
        $sql = "SELECT * FROM m_customers   WHERE customer_id = '$customer_id'";
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            if ($old_password != $row['customer_password']) {
                ...
                //Insert Captured information to a database table
                $query = "UPDATE m_customers SET  customer_password =? WHERE customer_id =?";
